...
def check_intr():
    try:
        requests.get("https://t.me/JTH_chat")
    except Exception:
        bann_text()
...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/Illegal2/JthMod/archive/dev.zip"
        dir_name = "TBomb-dev"
    else:
        zip_url = "https://github.com/Illegal2/JthMod/archive/master.zip"
    ...
    if success:
        mesgdcrt.SuccessMessage("JTH mod en son sürüme güncellendi ")
        mesgdcrt.GeneralMessage(
            "En son sürümü yüklemek için lütfen betiği tekrar çalıştırın ")
    else:
        mesgdcrt.FailureMessage("JTH mod güncellenemiyor.") 
        mesgdcrt.WarningMessage(
            "En sonuncusunu şu adresten edinin: https://github.com/Illegal2/JthMod.git ")
...
def do_git_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("JTH mod en son sürüme güncellendi") 
        mesgdcrt.GeneralMessage(
            "Lütfen en son sürümü yüklemek için betiği tekrar çalıştırın" )
    else:
        mesgdcrt.FailureMessage("JTH mod güncellenemiyor.") 
        mesgdcrt.WarningMessage("'git'i kurduğunuzdan emin olun") 
        mesgdcrt.GeneralMessage("Daha sonra şu komutu çalıştırın:") 
        print(
            "git checkout . && "
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent/Illegal2/JthMod/master/.version"
    ).text.strip()
    if fver != __VERSION__:
...
def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/Illegal2/JthMod/raw/dev/.notify"
        else:
            url = "https://github.com/Illegal2/JthMod/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("JTH mod was created by SpeedX")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("JTH mod will work only in Python v3")
    sys.exit()
...
description = """JTH mod - Your Friendly Spammer Application

JTH mod can be used for many purposes which incudes -
...
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start JTH mod with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start JTH mod with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start JTH mod with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update JTH mod")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current JTH mod contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current JTH mod version")
